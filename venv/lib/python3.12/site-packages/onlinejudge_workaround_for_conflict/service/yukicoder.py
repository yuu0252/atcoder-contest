...
class YukicoderContest(onlinejudge_workaround_for_conflict.type.Contest):
    """
...
class YukicoderProblem(onlinejudge_workaround_for_conflict.type.Problem):
    ...
    def download_system_cases(self, *, session: Optional[requests.Session] = None) -> List[TestCase]:
        ...
        resp = utils.request('GET', url, session=session)
        fmt = 'test_%e/%s'
        return onlinejudge_workaround_for_conflict._implementation.testcase_zipper.extract_from_zip(resp.content, fmt, ignore_unmatched_samples=True)  # NOTE: yukicoder's test sets sometimes contain garbages. The owner insists that this is an intended behavior, so we need to ignore them.
...
onlinejudge_workaround_for_conflict.dispatch.contests += [YukicoderContest]
