...
class HackerRankService(onlinejudge_workaround_for_conflict.type.Service):
...
class HackerRankProblem(onlinejudge_workaround_for_conflict.type.Problem):
    """
    ...
    def download_system_cases(self, *, session: Optional[requests.Session] = None) -> List[TestCase]:
        ...
        if resp.status_code == 403:
            ...
            raise onlinejudge_workaround_for_conflict.type.SampleParseError("Access Denied. Did you set your User-Agent?")
        resp.raise_for_status()
        return onlinejudge_workaround_for_conflict._implementation.testcase_zipper.extract_from_zip(resp.content, '%eput/%eput%s.txt')
...
onlinejudge_workaround_for_conflict.dispatch.services += [HackerRankService]
onlinejudge_workaround_for_conflict.dispatch.problems += [HackerRankProblem]
