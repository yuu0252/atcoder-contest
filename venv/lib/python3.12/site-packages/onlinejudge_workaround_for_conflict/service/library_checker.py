...
class LibraryCheckerService(onlinejudge_workaround_for_conflict.type.Service):
...
class LibraryCheckerProblem(onlinejudge_workaround_for_conflict.type.Problem):
    ...
    def download_sample_cases(self, *, session: Optional[requests.Session] = None) -> List[TestCase]:
        ...
        return onlinejudge_workaround_for_conflict._implementation.testcase_zipper.extract_from_files(iter(files))
...
onlinejudge_workaround_for_conflict.dispatch.services += [LibraryCheckerService]
onlinejudge_workaround_for_conflict.dispatch.problems += [LibraryCheckerProblem]
