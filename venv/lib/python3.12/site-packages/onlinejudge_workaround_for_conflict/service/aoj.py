...
class AOJService(onlinejudge_workaround_for_conflict.type.Service):
    ...
    def from_url(cls, url: str) -> Optional['AOJService']:
        ...
        # example: https://onlinejudge_workaround_for_conflict.u-aizu.ac.jp/home
...
class AOJProblem(onlinejudge_workaround_for_conflict.type.Problem):
    """
    ...
    def download_sample_cases(self, *, session: Optional[requests.Session] = None) -> List[TestCase]:
        ...
        if not samples:
            ...
            # list h3+pre
            zipper = onlinejudge_workaround_for_conflict._implementation.testcase_zipper.SampleZipper()
    ...
    def from_url(cls, url: str) -> Optional['AOJProblem']:
        ...
        # example: https://onlinejudge_workaround_for_conflict.u-aizu.ac.jp/challenges/sources/JAG/Prelim/2881
        # example: https://onlinejudge_workaround_for_conflict.u-aizu.ac.jp/courses/library/4/CGL/3/CGL_3_B
        ...
        # example: https://onlinejudge_workaround_for_conflict.u-aizu.ac.jp/problems/0423
        # example: https://onlinejudge_workaround_for_conflict.u-aizu.ac.jp/problems/CGL_3_B
...
class AOJArenaProblem(onlinejudge_workaround_for_conflict.type.Problem):
    """
    ...
    def from_url(cls, url: str) -> Optional['AOJArenaProblem']:
        # example: https://onlinejudge_workaround_for_conflict.u-aizu.ac.jp/services/room.html#RitsCamp19Day2/problems/A
        result = urllib.parse.urlparse(url)
        if result.scheme in ('', 'http', 'https') \
                and result.netloc == 'onlinejudge.u-aizu.ac.jp' \
...
onlinejudge_workaround_for_conflict.dispatch.services += [AOJService]
onlinejudge_workaround_for_conflict.dispatch.problems += [AOJProblem, AOJArenaProblem]
